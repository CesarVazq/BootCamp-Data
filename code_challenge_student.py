...
Original file is located at
    https://colab.research.google.com/drive/1zI4E8yqC_TEhmhMx31cuPqK0vSgYrr75
...
########## Q1
df2 = df.loc[df['Make']== 'Volkswagen', 'CO2 Emission Grams/Mile'].mean()
print(float(df2))
#Example answer:
#answer_dict["Q1"] =  11.54
...
########## Q2
#Your code here
df = pd.DataFrame(df,columns=['Make','Model'])
df.to_sql(
    'vehicules',
    conn,
    if_exists='replace')
...
query = cursor.execute(
    ...
    FROM vehicules
          GROUP BY Make
          ORDER BY COUNT(Model) DESC LIMIT 5
    """
)
top_5 = query.fetchall()
df = pd.DataFrame(top_5)
top_5 = df.values.tolist()
...
df.rename(columns = {'Fuel Type':'TypeFuel'}, inplace = True)

df = pd.DataFrame(df,columns=['TypeFuel'])

########## Q3
#Your code here
#df = pd.DataFrame(df,columns=['Fuel Type'])
df.to_sql(
    'vehicules',
    conn,
    if_exists='replace')
cursor_f = conn.cursor()
query_fuel = cursor.execute(
    """
    SELECT DISTINCT TypeFuel
      FROM vehicules 
       GROUP BY TypeFuel
       ORDER BY TypeFuel ASC
         
    """
)
fuel_type = query_fuel.fetchall()
fuel_type=[i[0] for i in fuel_type]
#df = pd.DataFrame(fuel_type)
#fuel_type = df.values.tolist()
alist = list(fuel_type)
print(alist)
...
file_name = "JulioCesar_TrejoVazquez_answers.pkl"
path = ""
